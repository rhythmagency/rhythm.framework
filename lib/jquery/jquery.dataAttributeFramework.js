		...
		var fn = function () {
			...
			*/
			$buttonDataShowOverlay.on(CONST.EVENTS.CLICK, function (e) {
				var $el = $(this).getElementByDataAttribute(CONST.DATA_ATTRIBUTES.SHOW_OVERLAY),
					data = $(this).data(CONST.DATA_ATTRIBUTES.SHOW_OVERLAY_DATA);

				$el.addClass(CONST.CSS.ACTIVE).trigger(CONST.EVENTS.RHYTHM.OVERLAY.SHOW, data);
			...
			*/
			$buttonDataHideOverlay.on(CONST.EVENTS.CLICK, function (e) {
				$(this).getElementByDataAttribute(CONST.DATA_ATTRIBUTES.HIDE_OVERLAY).removeClass(CONST.CSS.ACTIVE).trigger(CONST.EVENTS.RHYTHM.OVERLAY.HIDE);
				$body.css('overflow', '');
				e.preventDefault();
			});
